...
const CartItem: FC<CartItemProps> = ({ id, quantity }) => {
  ...
  return (
    <div className="bag-item h-20 w-20 bg-white rounded-xl relative">
      <img className="pt-1 w-9/12 mx-auto relative" src={item.photoPath} alt={'cart-item-img'} />
        <div className="absolute bg-amber-400 w-[25px] text-center rounded-[13px] top-16">{quantity}</div>
    </div>
  );
};
